...
# Category classification — looks up product_type from Products API

# —————————————————————————

def build_product_type_map(orders):
“”“Fetch product_type for all product_ids found in orders.”””
product_ids = set()
for order in orders:
...
for i in range(0, len(ids_list), 250):
    ...
    ids_param = ",".join(str(pid) for pid in chunk)
    resp = shopify_get("products", params={
        "ids": ids_param,
    ...
    if resp:
        for product in resp.json().get("products", []):
            product_type_map[product["id"]] = product.get("product_type", "")
    time.sleep(RATE_LIMIT_DELAY)
...
“”“Return product_type or None if excluded/blank.”””
pt = (product_type or “”).strip()
if not pt:
return None
if pt in EXCLUDED_PRODUCT_TYPES:
return None
return pt

def count_units_by_category(orders, product_type_map):
“”“Count units sold per product_type using the product type map.”””
counts = {}
for order in orders:
for item in order.get(“line_items”, []):
pid = item.get(“product_id”)
raw_type = product_type_map.get(pid, “”)
cat = classify_product_type(raw_type)
if cat:
counts[cat] = counts.get(cat, 0) + item.get(“quantity”, 0)
return counts

def get_sample_line_items(orders, product_type_map, n=10):
“”“Return sample line items for debugging.”””
samples = []
for order in orders:
for item in order.get(“line_items”, []):
pid = item.get(“product_id”)
...
“classified_as”: classify_product_type(raw_type),
})
if len(samples) >= n:
return samples
return samples
...
products[title] = products.get(title, Decimal(“0”)) + line_total
...
print("  Building product type map …")
product_type_map = build_product_type_map(yesterday_orders)
category_counts = count_units_by_category(yesterday_orders, product_type_map)
...
if args.slack:
    ...
    samples = get_sample_line_items(yesterday_orders, product_type_map, n=10)
    if samples:
